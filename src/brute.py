...
kimono=open("data/ugent.txt","r").read().splitlines()
kimono1=open("data/nokia1.txt","r").read().splitlines()
now = datetime.now()
day = now.day
month = now.month
year = now.year
...
try:
	...
	open('data/proxy1.txt','w').write(prox)
except Exception as e:
	print(' ')
prox=open('data/proxy1.txt','r').read().splitlines()
...
def metode_reguler(user, pwx, url):
	global ok,cp,loop
	ua = random.choice(kimono)
	ua2 = random.choice(kimono1)
	prox = open("data/proxy1.txt","r").read().splitlines()
